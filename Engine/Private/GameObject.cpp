...
CGameObject::CGameObject(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: m_pDevice(pDevice)
	, m_pContext(pContext)
	, m_bIsDead(false), m_bIsRender(true), m_bIsScroll(true)
	, m_eRenderGroup(CRenderer::RENDERGROUP::RENDER_PRIORITY)
	, m_iInstanceID(0)
	, m_pLayerTag(nullptr)
	, m_pNameTag(nullptr)
	, m_IsAllocatedCStringFlag(0)
{
...
CGameObject::CGameObject(const CGameObject& rhs)
	...
	, m_bIsDead(rhs.m_bIsDead), m_bIsRender(rhs.m_bIsRender), m_bIsScroll(rhs.m_bIsScroll)
	...
	, m_IsAllocatedCStringFlag(rhs.m_IsAllocatedCStringFlag)
{
	Safe_AddRef(m_pDevice);
	Safe_AddRef(m_pContext);
